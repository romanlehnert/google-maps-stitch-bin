...
describe Tiler::Location do

  describe "accessors" do

    let(:loc) { Tiler::Location.new }
  ...
  describe "#initialize" do
    context "without args" do
      ...
      it "returns a location object" do
        loc.should be_an_instance_of(Tiler::Location)
      end
    end


    context "given a hash with lat, lon and zoom" do
      let(:loc) { Tiler::Location.new(lat: 1, lon: 2, zoom: 12) }
    ...
    context "given a location" do
      ...
      places.each do |place|
        it "calculates the mercator projection of (lat: #{place[:lat]}, lon: #{place[:lon]}) to (meters x: #{place[:mx]}, meters y: #{place[:my]})" do
          Tiler::Location.new(lon: place[:lon], lat: place[:lat]).to_m.should eql([place[:mx], place[:my]])
        end
      end
    end
  end
    ...
    places =
      ...
      places.each do |place|
        it "calculates the mercator projection of (lat: #{place[:lat]}, lon: #{place[:lon]}) to (world coordinate x: #{place[:wx]}, world coordinate y: #{place[:wy]})" do
          Tiler::Location.new(lon: place[:lon], lat: place[:lat]).to_w.should eql([place[:wx], place[:wy]])
        end
      end
  ...
  describe "to_px" do
    places =
      [
        { zoom: 11, lat: 49.38237278700955, lon: 8.61328125, px: 274688,  py: 345088  },
        { zoom: 14, lat: 49.38237278700955, lon: 8.61328125, px: 2197504, py: 2760704 },
        { zoom: 14, lat: 49.49667452747044, lon: 8.96484375, px: 2201600, py: 2762752 },
      ]

    places.each do |place|
      it "calculates the pixels of (lat: #{place[:lat]}, lon: #{place[:lon]}) to (px: #{place[:px]}, py: #{place[:py]})" do
        Tiler::Location.new(lon: place[:lon], lat: place[:lat], zoom: place[:zoom]).to_px.should eql([place[:px], place[:py]])
      end
    end
  end
  ...
  describe "to_tile" do
    ...
    places.each do |place|
      it "calculates the tile of (lat: #{place[:lat]}, lon: #{place[:lon]}) to (tile x: #{place[:tx]}, tile y: #{place[:ty]})" do
        Tiler::Location.new(lon: place[:lon], lat: place[:lat], zoom: place[:zoom]).to_tile.should eql([place[:tx], place[:ty]])
      end
    end
  end
